...
The "Step 1: Load data" window offers an interface for loading a recording file, associated probe configuration file, setting processing parameters, and finally triggering the pre-processing of the data. The processed data is saved in a new ```toothy``` output folder.
...
<p align="center"><img src="_img/toothy_ingestion.png" width=60%/></p>
...
<p align="center"><img src="_img/toothy_array.png" width=40%/></p>

<hr>

### <ins>Assigning Probes</ins>

After the raw data source is loaded, each data row must be mapped to a unique probe channel. The central panel shows all probes currently associated with the recording.

<p align="center"><img align="center", src="_img/toothy_probe_assignment.png" width=60%/></p>

<p></p>
<ins>Load probe</ins>: load an existing probe configuration file<br>
<ins>Create/edit probe</ins>: open the probe designer window to make a new probe<br>
<ins>View row-to-probe assignment table</ins>: display a table with all data rows and their corresponding probe IDs; for unassigned rows, the "Probe" column is left blank

<br>

<b>For assigned probes:</b>
<p><img align="left", src="_img/copy.png" width=2%/>: duplicate assigned probe and add to the recording</p>
<p><img align="left", src="_img/clear-selection.png" width=2%/>: remove assigned probe from the recording</p>


<p></p>
<b>Set row indexing mode:</b><br>
<ins>Contiguous</ins>: each probe is assigned to a block of N consecutive rows in the data array<br>
<ins>Alternating</ins>: each probe is assigned to N rows evenly distributed throughout the array

### <ins>Setting Processing Parameters</ins>
After loading a probe configuration, the processing parameters for filtering and event detection must be set. A default set of parameters is automatically loaded, but users can also directly edit these values or upload custom files.
<p align="center"><img align="center", src="_img/toothy_parameters.png" width=60%/></p>

<br>

Finally, an <b>output directory</b> is set. The default is a folder called "toothy" located inside the same directory as the uploaded data.
...
<p align="center"><img align="center", src="_img/toothy_analysis_popup.png" width=60%/></p>
