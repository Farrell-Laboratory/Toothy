...
class DS_CSDWindow(QtWidgets.QDialog):
    ...
    def init_data(self, ddir, iprb, ishank):
        ...
        self.iprb = iprb
        self.ishank = ishank
        self.probe_list = h_io.read_probe_group(ddir).probes
        ...
        event_channels = h_io.load_event_channels(ddir, iprb, ishank=ishank)
        ...
        if dmode == 1: # load HDF5 files
            ...
            self.lfp_all = h_io.load_h5_lfp(self.FF, key='raw', iprb=iprb)[self.shank_channels, :]
            self.NOISE_TRAIN = h_io.load_h5_array(self.FF, 'NOISE', iprb, in_memory=True)[self.shank_channels]
        # elif dmode == 2:
        #     self.FF = None # load NPY/NPZ files
        #     self.lfp_time = np.load(Path(ddir, 'lfp_time.npy'))
        #     self.lfp_fs = int(np.load(Path(ddir, 'lfp_fs.npy')))
        #     self.lfp_all = h_io.load_bp(ddir, key='raw', iprb=iprb)[self.shank_channels, :]
        #     self.NOISE_TRAIN = h_io.load_noise_channels(ddir, iprb=iprb)[self.shank_channels]
        ...
        # load DS dataframe
        self.DS_DF = h_io.load_ds_dataset(ddir, iprb, ishank=ishank)
        ...
        self.CSD_PARAMS = dict(h_io.load_recording_params(ddir))
        if os.path.exists(CSDpath):  # CSD file exists for this recording
            ...
            with h5py.File(CSDpath, 'r+') as gg:
                if f'{K}/DS_DF' in gg:  # CSD exists for this probe and shank
                    ddf = h5df.load_df(gg.filename, f'{K}/DS_DF')
                    if len(ddf)==len(self.DS_DF) and all(ddf.idx==self.DS_DF.idx):
                        ...
                        self.CSD_PARAMS = dict(gg[K].attrs) # reading out the attributes?
    ...
    def get_csd(self, channels, idx, ddict):
        ...
        csd_obj = h_io.get_csd_obj(csd_lfp, self.coord_electrode[channels], ddict)
        csds = h_io.csd_obj2arrs(csd_obj)
        return (csd_lfp, *csds)
        
    def get_csd_surround(self, channels, idx, ddict, twin):
        """ Calculate the mean CSD surrounding DSs """
        iwin = int(round(twin*self.lfp_fs))
        mean_lfp = np.array([h_io.getavg(self.lfp_interp[i], idx, iwin) for i in channels])
        csd_obj = h_io.get_csd_obj(mean_lfp, self.coord_electrode[channels], ddict)
        mean_csds = h_io.csd_obj2arrs(csd_obj)
    ...
    def plot_csd_window(self, twin=0.2):
        ...
        # plot signals
        iwin = int(round(twin*self.lfp_fs))
        arr = np.array([h_io.getavg(self.lfp[i], self.iev, iwin) for i in self.channels])
    ...
    def plot_ds_by_type(self, twin):
        ...
        ds1_arr = np.array(h_io.getwaves(self.lfp[self.rel_hil_chan], self.idx_ds1, iwin))
        ds2_arr = np.array(h_io.getwaves(self.lfp[self.rel_hil_chan], self.idx_ds2, iwin))
    ...
    def save_csd(self):
        ...
        with h5py.File(Path(self.ddir, 'CSDs.hdf5'), 'a') as gg:
            ...
            h5df.save_df(gg.filename, f'{K}/DS_DF', self.DS_DF)
            gg[K].attrs.update(self.CSD_PARAMS)  # save params -- here the params are saved into the HDF5 attrs... TODO
