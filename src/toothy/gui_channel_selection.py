                ...
                if self.ADD_DS:
                    ...
                    try:
                        ...
                        ddict = dict(ch=self.hil_chan, time=self.lfp_time[imax], amp=amp,
                                     ...
                                     asym=h_io.get_asym(ipk,istart,istop), prom=prom,
                    ...
                    ifreq = h_io.get_inst_freq(hilb, self.lfp_fs, self.PARAMS.swr_freq)
        ...
        def on_press(event):
            ...
            ###   PLOT INSTANTANEOUS CSD   ###
                ...
                for i in noise_idx_rel:
                    ...
                    arr2[i,:] = interp_sig
                # calculate CSD
                csd_obj = h_io.get_csd_obj(arr2, self.coord_electrode, self.PARAMS)
                csd = h_io.csd_obj2arrs(csd_obj)[1]
                ...
                print("LEN??")
                print(len(self.lfp_time[imin:imax]))
                try:
...
class IFigEvent(QtWidgets.QWidget):
    ...
    def __init__(self, ch, channels, DF_ALL, DATA, lfp_time, lfp_fs, PARAMS, **kwargs):
        ...
        self.DF_MEAN = h_io.replace_missing_channels(self.DF_MEAN, self.channels)
    ...
    def create_subplots(self, twin=0.2):
        ...
        _ = h_io.plot_channel_events(self.DF_ALL, self.DF_MEAN,
    ...
    def rescale_y(self):
        ...
        if self.FLAG == 0:
            if len(self.CH_ON_PLOT) == 1:
                lfp_arr = h_io.getwaves(self.LFP, self.iev, self.iwin)
                yerrs, _ = h_io.getyerrs(lfp_arr, mode='sem')
                self.EY = pyfx.Limit(np.concatenate(yerrs), pad=0.05)
            else:
                waves = [h_io.getavg(self.LFP_arr[self.ch2irow[xch]],
            ...
            self.EY = pyfx.Limit(h_io.getwaves(self.LFP, self.iev, self.iwin).flatten(), pad=0.05)
    ...
    def plot_event_data(self, event, twin=None):
        ...
        if self.FLAG == 0:
            ...
            lfp_arr = h_io.getwaves(self.LFP, self.iev, self.iwin)
            (yerr0, yerr1), self.ev_y = h_io.getyerrs(lfp_arr, mode='sem')
            ...
            if len(self.CH_ON_PLOT) == 1:
                ...
                comparison_lines = []
                for xch in self.CH_ON_PLOT[1:]:
                    xmean = h_io.getavg(self.LFP_arr[self.ch2irow[xch]], 
                                         np.atleast_1d(self.DF_ALL.loc[xch].idx),
                                         self.iwin)
        ...
        ### plot individual events
            ...
            self.ev_y = h_io.pad_lfp(self.LFP, self.idx, self.iwin, pad_val=0)
    ...
    def plot_event_data(self, event, twin=None):
        ...
        if self.FLAG == 0:
            ...
            if self.SHOW_ENV == True:
                env_arr = h_io.getwaves(self.env, self.iev, self.iwin)
...
class ChannelSelectionWidget(QtWidgets.QFrame):
    ...
    def __init__(self, ddir, ntimes, lfp_time, lfp_fs, event_channels, parent=None):
        ...
        # load notes
        notes_txt = h_io.read_notes(Path(self.ddir, 'notes.txt'))
    ...
    def export_notes(self):
        ...
        h_io.write_notes(Path(self.ddir,'notes.txt'), txt)
        print('Notes saved!')
...
class ChannelSelectionWindow(QtWidgets.QDialog):
    ...
    """
    # Define custom signal to indicate the user has changed the event channel
    ch_signal = QtCore.pyqtSignal(int, int, int)

    def __init__(self, ddir, iprb=0, ishank=0, parent=None):
        # TODO change these variable names to be clearer
        """
        ddir: directory of processed data
        iprb: index of the probe
        ishank: index of the shank
        """
        super().__init__(parent)
        # TODO change the window-sizing to ensure it fits the window well (also for classificaton screen)
    ...
    def init_data(self, ddir, iprb, ishank=0):
        ...
        """
        # Path to the "Toothy output folder" (created by the Data Ingestion workflow)
        self.ddir = ddir 

        # Load the parameters used to process the recording
        self.PARAMS = h_io.load_recording_params(ddir)

        # Load the probeinterface "ProbeGroup" object and get the list of "Probe" objects
        self.probe_group = h_io.read_probe_group(ddir)
        self.probe_list = self.probe_group.probes
        self.n_probes = len(self.probe_list)
        # self.iprobes = list(range(len(self.probe_list)))

        # Form a list of lists (list of shanks for each probe)
        self.probe_shank_list = []
        for probe_object in self.probe_list:
            shank_list = probe_object.get_shanks() # list of "Shank" objects
            self.probe_shank_list.append(shank_list)
    
        # Load: LFP signals, event DataFrames, and detection thresholds for all probes
        ...
        self.lfp_time = h_io.load_h5_array(self.FF, 'lfp_time', in_memory=True)
        self.data_list = [h_io.load_h5_lfp(self.FF, iprb=i) for i in range(self.n_probes)]
        self.swr_list = [h_io.load_h5_event_df(self.FF, 'swr', i)[0] for i in range(self.n_probes)]
        self.ds_list = [h_io.load_h5_event_df(self.FF, 'ds', i)[0] for i in range(self.n_probes)]
        self.threshold_list = [h_io.load_h5_thresholds(self.FF, i) for i in range(self.n_probes)]
        self.noise_list = [h_io.load_h5_array(self.FF, 'NOISE', i, in_memory=True) for i in range(self.n_probes)]
        self.std_list = [h_io.load_h5_df(self.FF, 'STD', i) for i in range(self.n_probes)]
                        # elif dmode == 2:  # load NPZ and CSV files
                        #     self.FF = None
                        #     self.data_list, self.lfp_time, self.lfp_fs = h_io.load_lfp(ddir, '', -1)
                        #     self.swr_list = [x[0] for x in h_io.load_csv_event_dfs(ddir, 'swr', -1)]
                        #     self.ds_list = [x[0] for x in h_io.load_csv_event_dfs(ddir, 'ds', -1)]
                        #     self.threshold_list = list(np.load(Path(ddir, 'THRESHOLDS.npy'), allow_pickle=True))
                        #     self.noise_list = h_io.load_noise_channels(ddir, -1)
                        #     self.std_list = h_io.csv2list(ddir, 'channel_bp_std')
                        
        self.NTS = len(self.lfp_time)
        
        self.aux_array = h_io.load_aux(ddir)
        ...
        self.event_channel_list = [h_io.load_event_channels(ddir, i) for i in range(self.n_probes)]
        for i,l in enumerate(self.event_channel_list):
            for ii,ll in enumerate(l):
                if ll == [None,None,None]:
                    ...
                    DS_MEAN = h_io.get_mean_event_df(self.ds_list[i], self.std_list[i]).loc[shank_ch]
                    rel_noise_idx = np.nonzero(self.noise_list[i][shank_ch])[0]
                    itheta = h_io.estimate_theta_chan(STD, noise_idx=rel_noise_idx)
                    iripple = h_io.estimate_ripple_chan(STD, noise_idx=rel_noise_idx)
                    ihil = h_io.estimate_hil_chan(DS_MEAN, noise_idx=rel_noise_idx)
    ...
    def save_channels(self):
        ...
        if self.FF is not None:
            h_io.save_h5_df(self.SWR_ALL.drop('ch', axis=1), self.FF, name='ALL_SWR', iprb=self.iprb)
            h_io.save_h5_df(self.DS_ALL.drop('ch', axis=1), self.FF, name='ALL_DS', iprb=self.iprb)
            ...
            for event,DF in zip(['swr','ds'], [self.SWR_ALL, self.DS_ALL]):
                h_io.save_csv_event_dfs(self.ddir, event, DF, iprb=self.iprb)
        ...
        h_io.save_event_channels(self.ddir, iprb=self.iprb, ishank=self.ishank, 
