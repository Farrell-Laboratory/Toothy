...
class FileSelectionWidget(QtWidgets.QWidget):
    ...
    def __init__(self, title='', parent=None):
        ...
        self.ppath_btn = make_icon_button(QtGui.QIcon(':/resources/folder.png'))
...
class TableWidget(QtWidgets.QTableWidget):
    ...
    def load_df(self, df, static_columns=None, selected_columns=[]):
        ...
        if static_columns is not None:
            self.static_columns = static_columns
        print("STATIC COLS", self.static_columns)
        self.df = df
...
def make_icon_button(icon):
    """
    Return a QPushButton button with an icon on it

    icon: QtGui.QIcon object
    """
...
class ButtonPopup(QtWidgets.QDialog):
    """
    Simple pop-up with a few buttons. 
    Used in Data Ingestion step to allow the user to select:
     (1) a specific array (when multiple 2d arrays are found in a selected array file)
     (2) a specific ElectricalSeries (when multiple are found in a selected NWB file)
    """
    def __init__(self, *items, orientation='v', label='Select an option', title='', parent=None):
        super().__init__(parent)
        # Remove the "?" button
        ...
        # Add text to the top of the window
        self.label = QtWidgets.QLabel(label)

        # Create QPushButton widgets for each item
        self.btns = [QtWidgets.QPushButton(x) for x in items]

        # Style each button and connect the "clicked" signal to the "select_btn" slot
        button_ss = """
            QPushButton {
                background-color: white;
        ...
        # Make a widget container
        # note: arg "10" adds spacing between the label and the buttons
        layout = pyfx.get_widget_container(orientation, self.label, 5, *self.btns, cm=(15, 15, 15, 15), spacing = 5) #HERE
        self.setLayout(layout)
        
        # Show the QDialog and bring it to the top of the window stack
        self.show()
        self.raise_()
