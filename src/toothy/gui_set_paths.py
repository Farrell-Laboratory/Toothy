...
TODO Update resources_v2 to final file
"""
...
class SetPathsWidget(QtWidgets.QWidget):
    """
    Widget for setting paths for folders/files used in the Toothy workflow.
    Convenience widget; users can directly select any necessary folders/files in the standard workflow as well.
    """
    ...
    def __init__(self, parent=None):
        ...
        self.path_list = [str(x) for x in h_io.get_toothy_paths()]
    ...
    def make_row(self, row_idx, second_icon = None):
        ...
        row_elements = [path_display_le, gi.make_icon_button(QtGui.QIcon(icon_resource_path))]
        
        # Add a second button if one is requested
        if second_icon is not None:
            row_elements.append(gi.make_icon_button(QtGui.QIcon(f':/resources/{second_icon}.png')))
    ...
    def gen_layout(self):
        ...
        button_ss = """
            QPushButton {
                ...
                border: 1px solid #cccccc;
                border-radius: 5px;
                padding: 7px 0px; /* top and bottom padding, then left/right padding */
                font-weight: normal;
            }
            QPushButton:hover {
        ...
        """
        self.save_btn.setStyleSheet(button_ss)
    ...
    def choose_dir(self, row_idx):
        ...
        selected_dir_path = h_io.select_directory(current_directory, title = window_title, parent = self)
    ...
    def choose_probe_file(self):
        ...
        probe_object, file_path = h_io.select_load_probe_file(init_ppath = current_path, parent=self)
    ...
    def save_selections(self):
        ...
        """
        # Write to the file
        h_io.write_toothy_paths(self.path_list)
