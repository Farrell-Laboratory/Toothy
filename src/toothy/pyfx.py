...
def stealthy(widget, val):
    """
    Broad method for updating widgets without triggering signals.
    """
...
def get_widget_container(orientation, *widgets, widget='none', **kwargs):#✅
    ...
    # KNE change: removed groupbox option here since it doesn't appear to be used
        ...
        w = QtWidgets.QDialog() # Embed layout within a QDialog window
        w.setLayout(layout)
        return w
...
def center_window_vertically(widget):
    """
    Reposition window vertically on the screen, but keep horizontal positioning.
    """
    # Get the widget's window geometry (x, y, width, height) where x, y are the top left coordinates
    widget_rect = widget.frameGeometry()
    ...
    # Compute new y-position for vertical centering
    new_y = screen_rect.center().y() - widget_rect.height() // 2

    # Keep current x-position
    current_x = widget.x()

    # Move widget
    widget.move(current_x, new_y)

def center_window_upward(widget):
    """
    Move widget upward toward vertical center of the screen,
    but only if it is below the center (never moves it down).
    """
    widget_rect = widget.frameGeometry()
    screen_rect = widget.screen().availableGeometry()

    # Compute the ideal centered y-position
    centered_y = screen_rect.center().y() - widget_rect.height() // 2

    # Current position
    current_x = widget.x()
    current_y = widget.y()

    # Only move up if below center
    new_y = min(current_y, centered_y)
